...
public class SnapshotRangeTableInputFormat extends SnapshotInputFormat implements Configurable {
	private static final Logger LOGGER = Logger.getLogger(SnapshotRangeTableInputFormat.class);
	...
	public List<InputSplit> getSplits(JobContext context) throws IOException, InterruptedException {
		...
		printSplits(_splits);
		return convertFromTableToSnapshotSplit(_splits);
	}
	
	protected List<InputSplit> convertFromTableToSnapshotSplit(List<InputSplit> _splits) throws IOException {
		List<InputSplit> splits = new ArrayList<InputSplit>();
		for(InputSplit split : _splits) {
			TableSplit tsplit = (TableSplit)split;
			SnapshotRegionSplit newsplit = new SnapshotRegionSplit();
			newsplit.setEndRow(tsplit.getEndRow());
			newsplit.setStartRow(tsplit.getStartRow());
			newsplit.setScan(tsplit.getScan());
			splits.add(newsplit);
		}
		return splits;
	}
	...
	private Scan getScan() {
		Scan result = null;
		try {
			CustomizedUtil.convertStringToScan(conf.get(TableInputFormat.SCAN));
		} catch (Exception e) {
			result = new Scan();
		}
		return result;
	...
	public void setConf(Configuration configuration) {
		...
		if (hsql != null) {
			...
			if (counter) {
				Scan scan = new Scan();
				scan.setCaching(conf.getInt(TableInputFormat.SCAN_CACHEDROWS, 500));
		...
		try {
			setInput(configuration, tableName, new Path("/hbase/cdh94/weibo"));
		} catch (IOException e) {
