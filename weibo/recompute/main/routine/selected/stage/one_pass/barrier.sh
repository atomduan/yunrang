...
function is_stage_completed() {
	...
	r_log "[compute-one-pass-barrier-command] ${cmd}"
	result=$(eval ${cmd})
	if ! echo ${result} | grep -o _COMPLETE; then
		msg="[compute-barrier] ${task_name} Complete condition not met yet."
...
r_log "[compute-one-pass-barrier] ${task_name} Complete condition is met. Pass this barrier."
