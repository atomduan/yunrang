...
function is_stage_completed() {
	completed=0
	cmd="${BCMD_HADOOP} fs -ls ${ST_PUSH_MERGED_FIELD_RESULT}/_logs"
	...
	do
		...
		if ! echo ${result} | grep ${target_mark} > /dev/null; then
			msg="[push_merged_field_result_barrier]: Barrier condition not met on SERVER[${target_ip}]"
			msg="${msg}. There is no ${target_ip}_COMPLETE tag file under"
			msg="${msg} ${ST_PUSH_MERGED_FIELD_RESULT}/_logs"
			r_log "${msg}"
			completed=1
			break
		fi
...
cmd="${BCMD_HADOOP} fs -touchz ${ST_PUSH_MERGED_FIELD_RESULT}/_PUSH_COMPLETE"
r_log "${cmd}"
eval ${cmd}
r_log "[push_merged_field_result_barrier]: Barrier condition is met. Pass this barrier."
