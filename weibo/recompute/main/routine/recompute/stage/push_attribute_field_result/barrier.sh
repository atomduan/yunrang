...
function is_stage_completed() {
	completed=0
	cmd="${BCMD_HADOOP} fs -ls ${ST_PUSH_ATTRIBUTE_FIELD_RESULT}/_logs"
...
cmd="${BCMD_HADOOP} fs -touchz ${ST_PUSH_ATTRIBUTE_FIELD_RESULT}/_PUSH_COMPLETE"
r_log "${cmd}"
eval ${cmd}
r_log "[put_attribute_filed_result_barrier]: Barrier condition is met. Pass this barrier."
